...
func (this *UsersController) GetUsers() {
	...
	//authorizationBasic = utils.Substr(authorizationBasic, 6, len(authorizationBasic))
	...
	//return
	//}
	//md5String := fmt.Sprintf("%v%v%v", authUsername, authPasswd, string(time.Now().Unix())) // 需要加密的字符串为 UserName+UserPassword+时间戳
	//h := md5.New()

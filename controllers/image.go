...
func (this *ImageController) GetImageJson() {
	...
	returnImage := new(models.Image)
	_, err = models.Engine.Id(imageId).Get(returnImage)
...
func (this *ImageController) PutImageJson() {
	...
	_, errInsertOneImage := models.Engine.InsertOne(putRegistryImage)
...
func (this *ImageController) PutImageIdLayer() {
	...
	_, err = models.Engine.Id(returnImage.Id).Update(returnImage)
	//TODO: 判断 affected
	//判断Image记录是否更新成功
	if err != nil {
		this.Ctx.Output.Context.Output.Body([]byte("{\"error\": \"Image not found\"}"))
...
func (this *ImageController) PutChecksum() {
	...
	if ok {
		...
		parentImage := new(models.Image)
		_, err = models.Engine.Id(parentId).Get(parentImage)
		//TODO: Check has value
